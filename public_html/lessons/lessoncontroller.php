...
class LessonController {
    public static function get_lesson_by_id($id){
        ...
        $sql = "SELECT id, title, subject, description, resources, date_created FROM lessons WHERE id = ?";

        $statement = SQLAction::get_by_id_query($db, $sql, $id);

        $statement->bind_result($lessons_id, $title, $subject, $description, $resources, $date_created);

        $statement->fetch();

        $lesson = new Lesson($title, $subject, $description, $resources, 0, $date_created, "", $lessons_id);
    ...
    public static function get_all_lessons() {
        ...
        $sql = "SELECT id, title, subject, description, date_created FROM lessons";
    ...
    public static function add_lesson(Lesson $lesson){
        ...
        $sql = "INSERT INTO lessons (title, subject, description, resources, persons_id, date_created, search_field)  VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    public static function get_array_of_lessons($db, $statement) {
        $statement->bind_result($id, $title, $subject, $description, $date_created);

        $lessons = array();

        while ($statement->fetch()) {
            $lessons[] = new Lesson($title, $subject, $description, "", 0, $date_created, "", $id);
        }
